...
import { locale as english } from "app/main/users/visitors/i18n/en";
import { locale as arabic } from "app/main/users/visitors/i18n/ar";
...
export class VisitorListComponent implements OnInit {
   ...
   * @param {VisitorListService} _visitorListService
   */
  constructor(
  ...
  filterUpdate(event) {
    ...
    const temp = this.tempData.filter(function (d) {
      return (
        d.visitor.firstName.toLowerCase().indexOf(val) !== -1 ||
        d.visitor.lastName.toLowerCase().indexOf(val) !== -1 ||
        !val
      );
    });
    this.rows = temp;
   ...
   */
  async getRows() {
    await this._visitorListService.getDataTableRows().then((response) => {
   ...
   */
  addItem() {
    this.router.navigate([`/users/visitors/list/new`]);
  }

  editItem(row: any) {
    this.router.navigate([`/users/visitors/list/new/${row.id}`]);
  }

  deleteItem(id: string) {
    this.isLoading = true;
    this._visitorListService.deleteItem(id).then((response: any) => {
      this.isLoading = false;
      if (response.status) {
        Swal.fire({
          icon: "success",
