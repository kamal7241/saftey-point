...
export class CompoundsListComponent implements OnInit {
   ...
   * @param {CompoundsListService} _compoundsListService
   */
  constructor(
 ...
 */
  async getRows() {
    await this._compoundsListService.getDataTableRows().then(response => {
 ...
 */
  addItem() {
    this.router.navigate([`/users/compounds/list/new`]);
  }

  editItem(row: any) {
    this.router.navigate([`/users/compounds/list/new/${row.id}`]);
  }

  deleteItem(id: string) {
    this.isLoading = true;
    this._compoundsListService.deleteItem(id).then((response: any) => {
      this.isLoading = false;
      if (response.status) {
        Swal.fire({
          icon: 'success',
