...
export class CoorperatesListComponent implements OnInit {
  // Public
  public rows;
   ...
   * @param {coorperatesListService} _coorperatesListService
   */
  constructor(
 ...
 */
  addItem() {
    this.router.navigate([`/users/coorperates/list/new`]);
  }

  editItem(row: any) {
    this.router.navigate([`/users/coorperates/list/new/${row.id}`]);
  }

  deleteItem(id: string) {
    this.isLoading = true;
    this._coorperatesListService.deleteItem(id).then((response: any) => {
      this.isLoading = false;
      if (response.status) {
        Swal.fire({
          icon: 'success',
