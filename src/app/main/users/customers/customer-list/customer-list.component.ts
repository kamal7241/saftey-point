...
export class CustomerListComponent implements OnInit {
  ...
  filterUpdate(event) {
    ...
    const temp = this.tempData.filter(function (d) {
      return (
        d.customer.firstName.toLowerCase().indexOf(val) !== -1 ||
        d.customer.lastName.toLowerCase().indexOf(val) !== -1 ||
        !val
      );
    });
    this.rows = temp;
   ...
   */
  async getRows() {
    await this._customerListService.getDataTableRows().then((response) => {
   ...
   */
  addItem() {
    this.router.navigate([`/users/customers/list/new`]);
  }

  editItem(row: any) {
    this.router.navigate([`/users/customers/list/new/${row.id}`]);
  }

  deleteItem(id: string) {
    this.isLoading = true;
    this._customerListService.deleteItem(id).then((response: any) => {
      this.isLoading = false;
      if (response.status) {
        Swal.fire({
          icon: "success",
